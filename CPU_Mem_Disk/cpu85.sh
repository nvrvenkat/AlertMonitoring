...
# Function to create alert policy JSON
create_alert_policy_json() {
  local instance_id=$1
...
{
  "displayName": "$instance_name - CPU utilization 85%",
  "userLabels": {},
  "conditions": [
    {
      "displayName": "$instance_name - CPU utilization 85%",
      "conditionThreshold": {
        "filter": "resource.type = \"gce_instance\" AND resource.labels.instance_id = \"$instance_id\" AND metric.type = \"compute.googleapis.com/instance/cpu/utilization\"",
        "aggregations": [
          {
  ...
  for instance_id in "${instance_ids[@]}"; do
    ...
    if [ -n "$zone" ]; then
      ...
      if [ -n "$instance_name" ]; then
        local alert_policy_json=$(create_alert_policy_json "$instance_id" "$instance_name" "$notification_channel_id")
        local json_file="${instance_name}_alert_policy.json"
