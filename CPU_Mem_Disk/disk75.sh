...
# Function to create disk utilization alert policy JSON
create_disk_alert_policy_json() {
  local instance_id=$1
...
{
  "displayName": "$instance_name - Disk utilization 75%",
  "userLabels": {},
  "conditions": [
    {
      "displayName": "$instance_name - Disk utilization 75%",
  ...
  for instance_id in "${instance_ids[@]}"; do
    ...
    if [ -n "$zone" ]; then
      ...
      if [ -n "$instance_name" ]; then
        local alert_policy_json=$(create_disk_alert_policy_json "$instance_id" "$instance_name" "$notification_channel_id")
        local json_file="${instance_name}_disk_alert_policy.json"
